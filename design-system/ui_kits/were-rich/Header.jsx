...
function WRHeader({ total, fmtMoney, lastPayment, nextMilestone, toGo }) {
  const { Ticker, Badge } = window.WeReRichSetNForgetDesignSystem_60fa1b;
  return (
    <header style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      {/* brand row */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <img src="../../assets/setnforget-mark.png" alt="SetNForget" style={{ width: 34, height: 34, borderRadius: 9 }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 'var(--space-6)', alignItems: 'end' }}>
        <div>
          <h1 style={{
            fontFamily: 'var(--font-display)', textTransform: 'uppercase', margin: 0,
            fontSize: 'var(--fs-mega)', lineHeight: 0.88, letterSpacing: '-0.02em', color: 'var(--text-hi)',
          }}>
          ...
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: 'var(--fs-md)', color: 'var(--text-mid)', marginTop: 'var(--space-3)', maxWidth: 440, textWrap: 'pretty' }}>
            Every dollar Stripe sends us, live. {nextMilestone
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{
            ...
            fontSize: 'var(--fs-money)', lineHeight: 1, letterSpacing: '-0.01em',
            color: 'var(--green-400)', textShadow: 'var(--text-glow-green)',
          }}>{fmtMoney(total)}</div>
        </div>
      </div>
